...
void* acquire_page(mmaped_file_pool* mfp, uint64_t page_id)
{
	...
	if(fd == NULL) // need to create a new frame_desc for page_id
	{
		...
		if(frame == MAP_FAILED) // crash if mmap crashes
		{
			...
			// and try again
			frame = mmap64(NULL, mfp->page_size, PROT_READ | PROT_WRITE, MAP_SHARED, mfp->file->file_descriptor, page_id * mfp->page_size);
...
static void on_remove_all_from_page_id_to_frame_desc_hashmap_delete_frame_from_mmaped_file_pool(void* _mfp, const void* _fd)
{
...
void deinitialize_mmaped_file_pool(mmaped_file_pool* mfp)
{
	...
	remove_all_from_hashmap(&(mfp->page_id_to_frame_desc), &((notifier_interface){NULL, on_remove_all_from_page_id_to_frame_desc_hashmap_delete_frame_from_mmaped_file_pool}));
